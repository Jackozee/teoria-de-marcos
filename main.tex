...
\overfullrule = 5mm % marcar los overfull hbox
\usepackage{fancyhdr}
\pagestyle{fancy}
...
\usepackage{mathtools,thmtools,tikz-cd,multicol}
...
\usepackage[index, symbols]{glossaries}
\makeglossaries
\input{glosarios.tex}
\makeglossaries
...
\input{cba-ba}
\input{el problema de la reflexion booleana}
\input{Análisis de Hausdorff para marcos}
\input{Núcleos complementados y teoría descriptiva para marcos}
\input{Reflexiones pos.tex}
\input{productos-coproductos}
\input{tychonoff}
\input{producto-top-vs-coproducto-frm}
\input{gavillas}
\input{completaciones y sitios.tex}
\input{arboles}
\input{Dualidades}
\input{Separacion}
\input{Parches}
\input{Compactificaciones}
\input{Vietoris}
\input{Reales}
\input{padicos}
\input{algmodal}
